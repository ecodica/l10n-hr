...
class AccountMove(models.Model):
    ...
    joppd_posted = fields.Boolean(string='JOPPD Posted',
                                  readonly=True,
                                  copy=False,
                                  help='JOPPD Posted'
                                  )
    ...
    def post_joppd(self, post_date=None):
        ...
        for move in self.filtered(lambda m: m.is_for_joppd and m.journal_id.type in allowed_journal_types):
            date_payment = post_date or move.date
            if move.journal_id.type == 'general':
                date_payment = False
            for line in move.line_ids.filtered(lambda l: l.is_for_joppd):
                entry_vals = dict(
                    ...
                    nontaxable_receipt_id=line.joppd_nontaxable_receipt_id.id,
                    payment_method_id=line.joppd_payment_method_id.id,
                    amount=line.debit - line.credit,
                    company_id=move.company_id.id
                )
                joppd_entry_obj.create(entry_vals)
            move.joppd_posted = True
