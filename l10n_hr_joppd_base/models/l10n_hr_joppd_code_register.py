...
class L10nHrJOPPDCodeRegister(models.AbstractModel):
    ...
    parent_path = fields.Char(index=1, unaccent=False)
    ...
    name = fields.Char('Description', size=1024, required=True, index=1)
    code = fields.Char('Code', size=10, index=1)
    type = fields.Selection(TYPE_SELECTION, 'Type', required=True, default='normal')
    parent_id = fields.Many2one(comodel_name=_name,
                                string='Parent', index=1, ondelete='restrict', domain=[('type', '=', 'view')])
    child_ids = fields.One2many(comodel_name=_name, inverse_name="parent_id", string='Children', readonly=True)
