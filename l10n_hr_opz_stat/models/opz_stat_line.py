...
class OpzStatLine(models.Model):
    ...
    amount = fields.Float("Amount", required=1, default=0.0, digits=dp.get_precision('Account'))
    amount_tax = fields.Float("Amount Tax", required=1, default=0.0, digits=dp.get_precision('Account'))
    amount_total = fields.Float("Amount with Tax", required=1, default=0.0, digits=dp.get_precision('Account'))
    currency_id = fields.Many2one('res.currency', string='Currency', readonly=1)
    paid = fields.Float("Paid Amount", required=1, default=0.0, digits=dp.get_precision('Account'))
    unpaid = fields.Float("Unpaid Amount", required=1, default=0.0, digits=dp.get_precision('Account'))
    overdue_days = fields.Integer("Overdue Days", required=1, default=0, digits=dp.get_precision('Account'))
