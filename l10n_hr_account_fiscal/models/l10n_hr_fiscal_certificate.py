...
class L10nHrFiscalCertificate(models.Model):
    _name = "l10n_hr.fiscal.certificate"
    _inherit = ['mail.thread']
    _description = "Fiscal certificate store"
    ...
    def button_convert_p12(self):
        ...
        self.schema = self.cert_type == "demo" and "EDUC_v1.6" or "PROD_v1.6"
    ...
    def action_validate(self):
        for cert in self:
            if cert.not_after < fields.Datetime.now():
                ...
                cert.state = "active"
            if not cert.company_id.l10n_hr_fiscal_cert_id:
                cert.company_id.l10n_hr_fiscal_cert_id = cert
