...
class HrPayslip(models.Model):
    ...
    date = fields.Date('Datum računa', readonly=True,
        help="Ostavite prazno za koirištenje perioda prema datumu platne liste.")
    journal_id = fields.Many2one('account.journal', 'Dnevnik plaća', required=True,
         default=lambda self: self.env['account.journal'].search([('type', '=', 'general')], limit=1))
    move_id = fields.Many2one('account.move', 'Temeljnica', readonly=True, copy=False)
    ...
    def action_payslip_done(self):
        ...
        for slip in self:
            ...
            if currency.compare_amounts(credit_sum, debit_sum) == -1:
                ...
                if not acc_id:
                    raise UserError('Dnevnik troškova "%s" nema ispravno definiran potražni konto!' % (slip.journal_id.name))
                adjust_credit = (0, 0, {
                ...
                if not acc_id:
                    raise UserError('Dnevnik troškova "%s" nema ispravno definiran dugovni konto!' % (slip.journal_id.name))
                adjust_debit = (0, 0, {
                    'name': _('Adjustment Entry'),
                    'partner_id': False,
                    'account_id': acc_id,
...
class HrSalaryRule(models.Model):
    ...
    account_tax_id = fields.Many2one('account.tax', 'Porez')
    account_debit = fields.Many2one('account.account', 'Debitni račun', domain=[('deprecated', '=', False)])
    account_credit = fields.Many2one('account.account', 'Kreditni račun', domain=[('deprecated', '=', False)])

class HrContract(models.Model):
    _inherit = 'hr.contract'
    _description = 'Ugovor djelatnika'
...
class HrPayslipRun(models.Model):
    ...
    journal_id = fields.Many2one('account.journal', 'Dnevnik plaća', readonly=False,
