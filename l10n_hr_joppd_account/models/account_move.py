...
class AccountMove(models.Model):
    ...
    l10n_hr_joppd_posted = fields.Boolean(string='JOPPD Posted',
                                          readonly=True,
                                          copy=False,
                                          help='JOPPD Posted'
                                          )
    ...
    def post_joppd(self, post_date=None):
        ...
        for move in self.filtered(lambda m: m.l10n_hr_is_for_joppd and m.journal_id.type in allowed_journal_types):
            for line in move.line_ids.filtered(lambda l: l.l10n_hr_is_for_joppd):
                date_payment = False
                # filter out only for specific employee partner, resolving a possible case when move has more
                # than a single employee JOPPD line
                reconciled = line.move_id._get_reconciled_amls().filtered(lambda l: l.partner_id == line.partner_id)
                if reconciled:
                    date_payment = fields.first(reconciled).date
                entry_vals = dict(
